...
import { api, ApiError, type Driver } from "@/lib/api-client";
import { labels } from "@/lib/labels";
import { Field, inputClass } from "@/components/admin/form-field";

interface Props { initial?: Driver; }
type Status = "ACTIVE" | "INACTIVE";

export function DriverForm({ initial }: Props) {
  const router = useRouter();
  const isEdit = !!initial;
  const [name, setName] = useState(initial?.name ?? "");
  const [contact, setContact] = useState(initial?.contact ?? "");
  const [defaultFee, setDefaultFee] = useState<string>(initial?.defaultFee != null ? String(initial.defaultFee) : "");
  ...
  async function onSubmit(e: FormEvent) {
    ...
    if (defaultFee.trim()) body.defaultFee = Number(defaultFee);
    try {
      if (isEdit) await api.patch(`/drivers/${initial.id}`, body);
      else await api.post(`/drivers`, body);
      router.push("/admin/drivers?saved=1");
      router.refresh();
    } catch (err) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex max-w-xl flex-col gap-4">
      ...
      <Field label={labels.admin.fields.defaultFee}>
        <input type="number" inputMode="numeric" min={0} value={defaultFee} onChange={(e) => setDefaultFee(e.target.value)} className={inputClass} />
      </Field>
      ...
      <div className="flex gap-3">
        <button type="button" onClick={() => router.push("/admin/drivers")} className="rounded-lg border px-4 py-2">{labels.common.cancel}</button>
