...
export function EmployeeForm({ initial }: Props) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    if (monthlySalary.trim()) body.monthlySalary = Number(monthlySalary);
    try {
      if (isEdit) await api.patch(`/employees/${initial.id}`, body);
      else await api.post(`/employees`, body);
      router.push("/admin/employees?saved=1");
      router.refresh();
    } catch (err) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex max-w-xl flex-col gap-4">
      ...
      <Field label={labels.admin.fields.monthlySalary}>
        <input type="number" inputMode="numeric" min={0} value={monthlySalary} onChange={(e) => setMonthlySalary(e.target.value)} className={inputClass} />
      </Field>
