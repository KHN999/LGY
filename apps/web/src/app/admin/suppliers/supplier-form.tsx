...
import { api, ApiError, type Supplier } from "@/lib/api-client";
import { labels } from "@/lib/labels";
import { Field, inputClass } from "@/components/admin/form-field";

interface Props { initial?: Supplier; }
...
export function SupplierForm({ initial }: Props) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    try {
      if (isEdit) await api.patch(`/suppliers/${initial.id}`, body);
      else await api.post(`/suppliers`, body);
      router.push("/admin/suppliers?saved=1");
      router.refresh();
    } catch (err) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex max-w-xl flex-col gap-4">
      ...
      <div className="flex gap-3">
        <button type="button" onClick={() => router.push("/admin/suppliers")} className="rounded-lg border px-4 py-2">{labels.common.cancel}</button>
        <button
          type="submit"
